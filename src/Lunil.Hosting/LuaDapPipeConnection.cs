...
internal enum LuaDapMessageKind
{
    Request,
    Response,
    Event,
...
internal sealed record LuaDapMessage(LuaDapMessageKind Kind, string? Method, JsonNode? Body, int? Id)
{
    public static LuaDapMessage Request(JsonNode body, int id) => new(
        LuaDapMessageKind.Request,
        (string?)body["command"],
        body["arguments"],
        id);

    public static LuaDapMessage Response(int id, JsonNode body) => new(
        LuaDapMessageKind.Response,
        null,
        body,
        id);

    public static LuaDapMessage Event(string name, JsonNode? body = null) => new(
        LuaDapMessageKind.Event,
        name,
        body,
        null);
}
...
/// <summary>
/// Reads and writes DAP messages over a named pipe using the Content-Length framed JSON
/// transport shared with the debug adapter and LSP. Mirrors the adapter framing so the adapter
/// can relay requests and events verbatim between the client and the host.
/// </summary>
internal sealed class LuaDapPipeConnection : IDisposable
{
    ...
    public LuaDapPipeConnection(Stream input, Stream output)
    {
        _input = input;
        _output = output;
    }
    ...
    public LuaDapMessage? ReadMessage()
    {
        ...
        {
            return LuaDapMessage.Response(requestSequence.GetValue<int>(), body);
        }

        if (body["type"]?.GetValue<string>() == "event" && body["event"] is JsonNode eventName)
        {
            return LuaDapMessage.Event(eventName.GetValue<string>(), body["body"] as JsonObject);
        }

        throw new InvalidDataException($"Unsupported DAP message: {body}");
    }

    public void WriteMessage(LuaDapMessage message)
    {
        lock (_writeLock)
        {
    ...
    private void WriteMessageCore(LuaDapMessage message)
    {
        ...
        switch (message.Kind)
        {
            case LuaDapMessageKind.Response:
                body["type"] = "response";
            ...
            case LuaDapMessageKind.Event:
                ...
                break;
            case LuaDapMessageKind.Request:
                body["type"] = "request";
