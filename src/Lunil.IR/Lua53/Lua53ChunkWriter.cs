...
/// <summary>Writes chunks accepted by a matching PUC Lua 5.3 build.</summary>
public static class Lua53ChunkWriter
{
    ...
    private sealed class Writer
    {
        ...
        private readonly Lua53ChunkTarget _target;
        ...
        public void WriteChunk(Lua53Chunk chunk)
        {
            WriteBytes(Signature);
            WriteByte(0x53);
            WriteByte(0);
            WriteBytes(LuaData);
            WriteByte(_target.SizeOfInt);
            WriteByte(_target.SizeOfSizeT);
        ...
        private void WritePrototype(Lua53Prototype prototype, Lua53String? parentSource)
        {
        ...
        private void WriteConstant(Lua53Constant constant)
        {
            switch (constant.Kind)
            {
                case Lua53ConstantKind.Nil:
                    WriteByte(0);
                    break;
                case Lua53ConstantKind.False:
                    WriteByte(1);
                    break;
                case Lua53ConstantKind.True:
                    WriteByte(17);
                    break;
                case Lua53ConstantKind.Float:
                    WriteByte(3);
                    WriteNumber(constant.FloatValue);
                    break;
                case Lua53ConstantKind.Integer:
                    WriteByte(19);
                    WriteInteger(constant.IntegerValue);
                    break;
                case Lua53ConstantKind.ShortString:
                    WriteByte(4);
                    WriteString(constant.StringValue);
                    break;
                case Lua53ConstantKind.LongString:
                    WriteByte(20);
                    WriteString(constant.StringValue);
                    break;
                default:
        ...
        private void WriteString(Lua53String? value)
        {
            if (value is null)
            {
                WriteSize(0);
        ...
        private void WriteSize(ulong value)
        {
            ...
            if (_target.SizeOfSizeT == 4)
            {
                if (value > uint.MaxValue)
                {
                    throw new InvalidOperationException("Lua 5.3 size_t value exceeds 32-bit target.");
                }
            ...
            else if (_target.ByteOrder == Lua53ByteOrder.LittleEndian)
            {
            ...
            WriteBytes(bytes[.._target.SizeOfSizeT]);
        }
