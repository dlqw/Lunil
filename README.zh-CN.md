...
> [!IMPORTANT]
> 当前源码版本是 **`0.7.0-alpha.4`**。`0.6.0` 开发线已在不可变的
> `0.6.0-alpha.14` 执行后端预览结束，不再发布无后缀 `0.6.0`。本里程碑新增公共
> Compiler/Hosting 产品边界、LuaLS/legacy EmmyLua 注解前端、有界类型/控制流分析和增量
> module workspace 已经可用；CLI 与完整 conformance/API 门槛仍属于 Alpha 开发内容。
...
| CLI | 后续 0.7 Alpha 计划 | `run`/`check`/`build`/`dump`、配置、响应文件、统一诊断和 deterministic/sandbox 选项 |
| 稳定性契约 | Alpha 预发布 | 当前构建适合评估与集成测试；`1.0.0` 前仍可能发生破坏性 API 更新 |

### 当前后端就绪情况

| 执行路径 | Release 行为 | 进入 `0.7.0-alpha.4` 时继承的就绪情况 |
| --- | --- | --- |
