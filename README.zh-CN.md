...
<p align="center">
  ...
  <a href="https://github.com/dlqw/Lunil/releases"><img alt="版本" src="https://img.shields.io/badge/version-0.6.0--alpha.14-7c3aed?style=flat-square"></a>
...
> [!IMPORTANT]
> 当前源码版本是 **`0.6.0-alpha.14`**。编译器、托管运行时、标准库以及已通过资格验证的
> JIT/AOT 路径均已可用，并有六 RID CI 覆盖。当前代码适合发布 **alpha 预发布构建**，
> 但还不能标记为 beta、RC 或无后缀稳定版：公共 API、完整 Lua 官方测试套件覆盖以及
> 长期兼容性/支持契约尚未冻结。
...
| 执行路径 | Release 行为 | `0.6.0-alpha.14` 就绪情况 |
| --- | --- | --- |
...
这些结果完成了当前 JIT/AOT 性能生产化里程碑，但不等于整个产品已经冻结。进入 beta
还要求冻结 `0.6.0` 的功能与 API 范围；进入稳定版还必须先通过 RC 及其兼容性准入标准。
...
当前发布结论是 **`0.6.0-alpha.14`**：JIT/AOT 性能生产化已经通过资格验证，但完整的
`0.6.0` 功能/API 范围尚未冻结，因此改成 `beta`、`rc` 或移除后缀都会夸大稳定性。
同一 channel 中每发布一个构建都递增预发布序号；切换到新 channel 时从 `1` 开始。
版本一旦打 tag 或发布就不可变，后续修复必须使用下一个编号，例如 `alpha.15`。
