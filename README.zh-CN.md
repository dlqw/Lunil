...
- **面向多层执行后端**：reference interpreter、持久化 CIL AOT 与 CoreCLR Tier 1 JIT
  已共享同一套 verified execution contract；Tier 2 与 NativeAOT 构建集成继续复用该 ABI。
...
| JIT / AOT 后端 | 开发中 | persisted CIL AOT v1 与 policy-driven CoreCLR Tier 1 JIT 已实现；Tier 2 与构建期 NativeAOT 待完成 |
