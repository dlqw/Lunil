...
<p align="center">
  ...
  <a href="https://github.com/dlqw/Lunil/releases"><img alt="版本" src="https://img.shields.io/badge/version-0.6.0--alpha.8-7c3aed?style=flat-square"></a>
...
> [!IMPORTANT]
> Lunil 当前版本是 **`0.6.0-alpha.8`**。编译器、托管运行时和完整 Lua 5.4 标准库
...
- **面向多层执行后端**：reference interpreter、持久化 CIL AOT 与 profile-guided
  CoreCLR Tier 1/Tier 2 JIT、实验性 loop OSR 已共享同一套 verified execution
...
| JIT / AOT 后端 | 预览 | 已通过资格验证的 Tier 1 由 `Auto` 默认启用；精确数值 profile 的 Tier 2 会生成带 guard 的专门化 CIL，非数值 Tier 2 与 loop OSR 在更广门槛通过前仍需显式开启 |
...
继续精确回退解释器。设置 `EnableTier2=true` 后，稳定的整数、浮点与混合数值 profile 会生成
带 guard 的专门化 CIL；guard 失败时精确 deopt 回 canonical execution。其他 Tier 2 profile
形态仍使用 managed fallback，因此 Tier 2 保持显式 opt-in。Loop OSR 则独立通过
`EnableLoopOsr=true` 开启。
