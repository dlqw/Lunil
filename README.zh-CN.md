...
> [!IMPORTANT]
> 当前源码版本是 **`0.8.0-beta.1`**；稳定版 `0.7.0` 及其 API/package baseline 保持
> 不可变。本 Beta 冻结已完成的 interpreter、Tier 1/Tier 2 JIT、loop OSR、公共 API、
> assembly、package、IR/profile 和证据 schema 范围。Lua persisted/static AOT 继续保持移除，
> .NET NativeAOT 解释器兼容继续受支持。后续 `0.8` 预发布版只接受兼容性、诊断、
> 文档、可靠性和性能收敛修复，全部发布门槛仍然必须通过。
...
| 执行后端 | `0.8` Beta | 合格 interpreter、Tier 1/Tier 2 JIT 与 loop OSR；Lua persisted/static AOT 已在 `0.8.0-alpha.12` 移除 |
...
| 稳定性契约 | 活跃 Beta | 稳定 `0.7.0` 保持冻结；`0.8.0-beta.1` 的功能/API/package 范围已冻结，只能经验收通过的 RC 晋级 |

### 当前后端就绪情况

| 执行路径 | Release 行为 | `0.8.0-beta.1` 就绪情况 |
| --- | --- | --- |
...
稳定 `0.7.0` 证据继续作为回归下限。`0.8.0-beta.1` 在不改变稳定 `api/0.7.0`
契约的前提下冻结其[更新日志](changelogs/0.8.0-beta.1.md)所述范围。Lua AOT 移除等
...
当前源码版本是 **`0.8.0-beta.1`**。稳定版 `0.7.0`、其 tag 与 `api/0.7.0` 保持
不可变；该稳定线的向后兼容修复使用 `0.7.1`。完整 `0.8` 功能、公共 API、assembly
和 package 范围现已冻结在 `api/0.8.0` 中。Beta 只接受兼容性、诊断、文档、可靠性和性能
收敛修复，之后才能晋级 `rc.1`。
