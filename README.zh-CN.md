...
<p align="center">
  ...
  <a href="https://github.com/dlqw/Lunil/releases"><img alt="版本" src="https://img.shields.io/badge/version-0.7.0-7c3aed?style=flat-square"></a>
...
> [!IMPORTANT]
> 当前源码版本是稳定版 **`0.7.0`**。`0.6.0` 开发线已在不可变的
...
> 通过，14 个公共 assembly/package 已由兼容性基线冻结。审核通过的发布候选在晋级
> 稳定版之前未修改产品代码、公共 API baseline 或 package 范围。
...
| JIT / AOT 后端 | 稳定 | Tier 1、精确数值 Tier 2 与带守卫的精确数值 loop OSR 已具备六 RID rollout 证据并默认自动启用；persisted CIL 已具备可验证、可回收的加载/执行路径与六 RID 生产性能门槛；managed semantic fallback 仍是实验性显式 opt-in |
...
| 稳定性契约 | 稳定版 | 本系列的向后兼容修复使用 `0.7.1`；下一个功能/API 里程碑是 `0.8.0` |

### 当前后端就绪情况

| 执行路径 | Release 行为 | 稳定版 `0.7.0` 已接受的就绪情况 |
| --- | --- | --- |
...
回归门槛。稳定版保留[0.7.0 路线图](docs/roadmap-0.7.0.md)中已冻结的编译器、分析、
宿主、CLI、conformance 与 package 产品面。
...
当前版本是稳定版 **`0.7.0`**。`0.6.0` 开发线已明确在
`0.6.0-alpha.14` 结束；已发布 tag 保持不可变，不会再创建无后缀 `0.6.0`。
同一 channel 中每发布一个构建都递增预发布序号；切换到新 channel 时从 `1` 开始。
已接受的 Beta 进入 RC，审核通过的 `0.7.0-rc.1` 又进入稳定版，整个过程未修改产品代码或
冻结的 API/package 范围。向后兼容修复使用 `0.7.1`；新功能/API 开发从
`0.8.0-alpha.1` 开始。
