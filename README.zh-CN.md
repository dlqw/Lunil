...
<p align="center">
  ...
  <a href="https://github.com/dlqw/Lunil/releases"><img alt="版本" src="https://img.shields.io/badge/version-0.6.0--alpha.10-7c3aed?style=flat-square"></a>
...
> [!IMPORTANT]
> Lunil 当前版本是 **`0.6.0-alpha.10`**。编译器、托管运行时和完整 Lua 5.4 标准库
...
| JIT / AOT 后端 | 预览 | 已通过资格验证的 Tier 1 与精确数值 Tier 2 默认自动启用；带守卫的精确数值 loop OSR 已生产化但仍需显式开启，所有 managed fallback 路径也保持显式 opt-in |
...
`GuardedExactNumericCil` 的 verified natural loop；load/move、控制流、numeric-for、guarded close
以及精确整数、浮点和混合数值操作会在生成的循环方法中执行，同时保留 canonical PC、预算、
hook/debug、GC 与 deopt 守卫。table、closure、upvalue、call、vararg 和 to-be-closed 语义边界会在
编译前拒绝。只有显式设置 `EnableLoopOsrManagedFallback=true` 才恢复实验性的 managed canonical
loop 路径。动态代码不可用时不会分析、预热或编译 OSR。
