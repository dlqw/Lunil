...
<p align="center">
  ...
  <a href="https://github.com/dlqw/Lunil/releases"><img alt="版本" src="https://img.shields.io/badge/version-0.7.0--alpha.1-7c3aed?style=flat-square"></a>
...
> [!IMPORTANT]
> 当前源码版本是 **`0.7.0-alpha.1`**。`0.6.0` 开发线已在不可变的
> `0.6.0-alpha.14` 执行后端预览结束，不再发布无后缀 `0.6.0`。本里程碑新增公共
> `Lunil.Compiler` 与 `Lunil.Hosting` 产品边界；LuaLS/EmmyLua 注解、类型/流/模块分析、
...
| 静态分析与 CLI | 0.7 Alpha 计划 | LuaLS/legacy EmmyLua 注解、类型/流/模块分析、增量 workspace 与 `run`/`check`/`build`/`dump` CLI |
| 稳定性契约 | Alpha 预发布 | 当前构建适合评估与集成测试；`1.0.0` 前仍可能发生破坏性 API 更新 |

### 当前后端就绪情况

| 执行路径 | Release 行为 | 进入 `0.7.0-alpha.1` 时继承的就绪情况 |
| --- | --- | --- |
