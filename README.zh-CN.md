...
| Workspace 产品 API | Alpha 基础 | 稳定 module/source identity、可注入 resolver、静态/动态 require 分类、SCC fixed point、内容寻址缓存、最小失效、有界并行、取消与确定性合并 |
| CLI | Alpha 产品 | 已打包的 `lunil` 工具，提供 `run`/`check`/`build`/`dump`、稳定退出码、text/JSON 诊断、stdin、响应文件、分层配置、workspace 解析、资源预算与 trusted/sandbox/deterministic profile |
| 稳定性契约 | Alpha 预发布 | 当前构建适合评估与集成测试；`1.0.0` 前仍可能发生破坏性 API 更新 |

### 当前后端就绪情况

| 执行路径 | Release 行为 | 进入 `0.7.0-alpha.5` 时继承的就绪情况 |
| --- | --- | --- |
