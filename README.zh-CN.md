...
> 当前源码树为 `0.12.0-alpha.24` 热更新预览，新增协调式 target 流量隔离、有界 prepare
...
- **生产热更新预览**：支持 key 轮换与撤销的签名 Patch Bundle、由 signer 授权的回滚、capability
  ...
  持久恢复 journal、可配置的 stale-generation budget、不持有 Lua heap graph 的有界 rollout history，
  以及 .NET telemetry。
