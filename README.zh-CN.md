...
> [!IMPORTANT]
> 当前源码版本是 **`0.8.0-alpha.9`**；稳定版 `0.7.0` 及其 API/package baseline 保持
...
| 稳定性契约 | 活跃 Alpha | 稳定 `0.7.0` 保持冻结；`0.8.0-alpha.9` 允许已审核功能/API 工作，不能直接晋级稳定版 |

### 当前后端就绪情况

| 执行路径 | Release 行为 | `0.8.0-alpha.9` 就绪情况 |
| --- | --- | --- |
...
| Persisted CIL AOT | 显式 artifact 编译、验证、collectible load 与执行 | 六个发布 RID 均已通过；exact-module profile 可持久化带守卫的 unboxed 数值区域并精确回退 |
| 构建期 AOT / NativeAOT | 使用 `Lunil.Build` 时生成静态 registry；动态模块回退解释器 | 六个发布 RID 的 build/publish 集成均已验证 |

稳定 `0.7.0` 证据继续作为回归下限。`0.8.0-alpha.9` 在不改变冻结 `api/0.7.0` 契约的
前提下叠加其[更新日志](changelogs/0.8.0-alpha.9.md)所述工作。
...
```bash
dotnet tool install --global Lunil.Cli --version 0.8.0-alpha.9
lunil --version
...
```xml
<PackageReference Include="Lunil.Hosting" Version="0.8.0-alpha.9" />
```
...
执行从当前 canonical PC 精确回退。通过 `LuaAotCompilationOptions.Profile` 传入 exact-module
`LuaJitModuleProfile` 后，metadata PE 会持久化与 Tier 2 共用的 backend-neutral 数值区域 IL，并使用
unboxed integer、floating-point 与 Boolean local。profile fingerprint 和策略版本进入 artifact
identity；陈旧或畸形 profile 会 fail closed，guard mismatch 或预算 poll 则从精确 canonical PC 恢复。
...
```xml
<ItemGroup>
  <PackageReference Include="Lunil.Build" Version="0.8.0-alpha.9" />
...
当前源码版本是 **`0.8.0-alpha.9`**。稳定版 `0.7.0`、其 tag 与 `api/0.7.0` 保持
...
| [Persisted profile-guided 数值区域](docs/adr/0015-persisted-profile-guided-numeric-regions.md) | Exact-module profile identity、共享数值 IL、守卫、safepoint 与 artifact 兼容性 |
