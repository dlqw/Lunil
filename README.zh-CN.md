...
<p align="center">
  ...
  <a href="https://github.com/dlqw/Lunil/releases"><img alt="版本" src="https://img.shields.io/badge/version-0.8.0--alpha.12-7c3aed?style=flat-square"></a>
...
> [!IMPORTANT]
> 当前源码版本是 **`0.8.0-alpha.12`**；稳定版 `0.7.0` 及其 API/package baseline 保持
> 不可变。本 Alpha 移除 Lua persisted/static AOT 产品与 `Lunil.Build`，保留 .NET NativeAOT
> 下的解释器兼容，并完成剩余 JIT 后端的 64 位指令计数。`api/0.8.0` 是已审核快照而非
> Beta freeze，完整 conformance、package、六 RID 与性能门槛仍然必须通过。
...
| 执行后端 | `0.8` Alpha | 合格 interpreter、Tier 1/Tier 2 JIT 与 loop OSR；Lua persisted/static AOT 已在 `0.8.0-alpha.12` 移除 |
...
| 稳定性契约 | 活跃 Alpha | 稳定 `0.7.0` 保持冻结；`0.8.0-alpha.12` 允许已审核功能/API 工作，不能直接晋级稳定版 |

### 当前后端就绪情况

| 执行路径 | Release 行为 | `0.8.0-alpha.12` 就绪情况 |
| --- | --- | --- |
...
稳定 `0.7.0` 证据继续作为回归下限。`0.8.0-alpha.12` 在不改变冻结 `api/0.7.0` 契约的
前提下叠加其[更新日志](changelogs/0.8.0-alpha.12.md)所述工作。
...
当前源码版本是 **`0.8.0-alpha.12`**。稳定版 `0.7.0`、其 tag 与 `api/0.7.0` 保持
