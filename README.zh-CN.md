...
> [!IMPORTANT]
> 当前源码版本是 **`0.8.0-alpha.13`**；稳定版 `0.7.0` 及其 API/package baseline 保持
> 不可变。本 Alpha 增加紧凑 Tier 0 解释循环，同时保留精确 debug、预算、协程、GC/finalizer
> 与后端切换语义。Lua persisted/static AOT 继续保持移除，.NET NativeAOT 解释器兼容继续
> 受支持。`api/0.8.0` 是已审核快照而非 Beta freeze，全部发布门槛仍然必须通过。
...
| 稳定性契约 | 活跃 Alpha | 稳定 `0.7.0` 保持冻结；`0.8.0-alpha.13` 允许已审核功能/API 工作，不能直接晋级稳定版 |

### 当前后端就绪情况

| 执行路径 | Release 行为 | `0.8.0-alpha.13` 就绪情况 |
| --- | --- | --- |
...
稳定 `0.7.0` 证据继续作为回归下限。`0.8.0-alpha.13` 在不改变冻结 `api/0.7.0` 契约的
前提下叠加其[更新日志](changelogs/0.8.0-alpha.13.md)所述工作。
...
当前源码版本是 **`0.8.0-alpha.13`**。稳定版 `0.7.0`、其 tag 与 `api/0.7.0` 保持
