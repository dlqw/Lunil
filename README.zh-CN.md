...
> [!IMPORTANT]
> 当前源码版本是 **`0.7.0-alpha.3`**。`0.6.0` 开发线已在不可变的
> `0.6.0-alpha.14` 执行后端预览结束，不再发布无后缀 `0.6.0`。本里程碑新增公共
> Compiler/Hosting 产品边界、LuaLS/legacy EmmyLua 注解前端和有界类型/控制流分析已经可用；
> 模块/增量 workspace 与 CLI 仍属于 Alpha 开发内容，`0.7.0` 范围和晋级门槛尚未冻结。
...
| JIT / AOT 后端 | Alpha 已验证 | Tier 1、精确数值 Tier 2 与带守卫的精确数值 loop OSR 已具备六 RID rollout 证据并默认自动启用；persisted CIL 已具备可验证、可回收的加载/执行路径与六 RID 生产性能门槛；managed semantic fallback 仍是实验性显式 opt-in |
| Compiler 产品 API | Alpha 基础 | 统一的有界 lex/parse/bind/lower/verify 管线、不可变结果、阶段诊断、取消边界和 canonical source identity |
| Hosting 产品 API | Alpha 基础 | 可复用 compile/execute 宿主、Trusted/Restricted/Deterministic 能力 profile 和运行时预算 |
| 注解产品 API | Alpha 基础 | 共享有界 annotation lexer/type AST、LuaLS 默认 parser、legacy EmmyLua 兼容、未知标签保留、可配置诊断和 suppression |
| 类型与流分析 API | Alpha 基础 | semantic type/type-pack、annotation declaration、constraint、CFG、函数/返回推断、nil/type/assert/判别字段收窄、definite assignment、unreachable、generic、source suppression 和确定性 widening 预算 |
| Workspace 与 CLI | 后续 0.7 Alpha 计划 | 模块图、内容寻址增量 workspace 与 `run`/`check`/`build`/`dump` CLI |
| 稳定性契约 | Alpha 预发布 | 当前构建适合评估与集成测试；`1.0.0` 前仍可能发生破坏性 API 更新 |

### 当前后端就绪情况

| 执行路径 | Release 行为 | 进入 `0.7.0-alpha.3` 时继承的就绪情况 |
| --- | --- | --- |
...
回归门槛。当前 Alpha 转向收口[0.7.0 路线图](docs/roadmap-0.7.0.md)中的编译器、分析、
宿主和 CLI 产品面。
...
`LuaCompiler`、`LuaExecutor` 以及各 syntax、annotation、analysis、semantic、IR 底层包仍可独立
...
该 package 在 `CoreCompile` 前验证并编译所有模块，将确定性 artifact 写入 `obj/lunil/`，
并生成不依赖 runtime reflection 的直接 method registry。宿主可取得内嵌 canonical module，
再执行其静态入口：
