...
<p align="center">
  ...
  <a href="https://github.com/dlqw/Lunil/releases"><img alt="版本" src="https://img.shields.io/badge/version-0.7.0--rc.1-7c3aed?style=flat-square"></a>
...
> [!IMPORTANT]
> 当前源码版本是 **`0.7.0-rc.1`**。`0.6.0` 开发线已在不可变的
> `0.6.0-alpha.14` 执行后端预览结束，不再发布无后缀 `0.6.0`。本里程碑新增公共
> Compiler/Hosting 产品边界、LuaLS/legacy EmmyLua 注解前端、有界类型/控制流分析、增量
> module workspace 与 `lunil` CLI 已经可用；官方 Lua 5.4.8 user-mode suite 与六 RID 证据已
> 通过，14 个公共 assembly/package 已由兼容性基线冻结。该发布候选在稳定版
> `0.7.0` 之前只接受阻断发布的问题修复。
...
| JIT / AOT 后端 | RC 已验证 | Tier 1、精确数值 Tier 2 与带守卫的精确数值 loop OSR 已具备六 RID rollout 证据并默认自动启用；persisted CIL 已具备可验证、可回收的加载/执行路径与六 RID 生产性能门槛；managed semantic fallback 仍是实验性显式 opt-in |
| Compiler 产品 API | RC 已冻结 | 统一的有界 lex/parse/bind/lower/verify 管线、不可变结果、阶段诊断、取消边界和 canonical source identity |
| Hosting 产品 API | RC 已冻结 | 可复用 compile/execute 宿主、Trusted/Restricted/Deterministic 能力 profile 和运行时预算 |
| 注解产品 API | RC 已冻结 | 共享有界 annotation lexer/type AST、LuaLS 默认 parser、legacy EmmyLua 兼容、未知标签保留、可配置诊断和 suppression |
| 类型与流分析 API | RC 已冻结 | semantic type/type-pack、annotation declaration、constraint、CFG、函数/返回推断、nil/type/assert/判别字段收窄、definite assignment、unreachable、generic、source suppression 和确定性 widening 预算 |
| Workspace 产品 API | RC 已冻结 | 稳定 module/source identity、可注入 resolver、静态/动态 require 分类、SCC fixed point、内容寻址缓存、最小失效、有界并行、取消与确定性合并 |
| CLI | RC 已冻结 | 已打包的 `lunil` 工具，提供 `run`/`check`/`build`/`dump`、稳定退出码、text/JSON 诊断、stdin、响应文件、分层配置、workspace 解析、资源预算与 trusted/sandbox/deterministic profile |
| 稳定性契约 | 发布候选 | 稳定版 `0.7.0` 之前只允许阻断发布的问题修复 |

### 当前后端就绪情况

| 执行路径 | Release 行为 | `0.7.0-rc.1` 已接受的就绪情况 |
| --- | --- | --- |
...
这些结果已完成 `0.6.0-alpha.14` 的 JIT/AOT 性能生产化里程碑，并继续作为 `0.7.0`
回归门槛。RC 保留[0.7.0 路线图](docs/roadmap-0.7.0.md)中已冻结的编译器、分析、宿主、
CLI、conformance 与 package 产品面，并且只接受阻断发布的问题修复。
...
Restricted；Trusted 能力和确定性执行都必须显式选择：
...
当前开发版本是 **`0.7.0-rc.1`**。`0.6.0` 开发线已明确在
`0.6.0-alpha.14` 结束；已发布 tag 保持不可变，不会再创建无后缀 `0.6.0`。
同一 channel 中每发布一个构建都递增预发布序号；切换到新 channel 时从 `1` 开始。
已接受的 Beta 未修改产品代码或冻结的 API/package 范围便进入 RC。`0.7.0-rc.1`
打 tag 后，只有阻断发布的问题才能产生 `0.7.0-rc.2`；通过验收且未变更的 RC 将晋级为
稳定版 `0.7.0`。
...
| [0.7.0 API/package 兼容性](docs/api-compatibility.md) | 已冻结 assembly API、NuGet 资产、验证规则与更新命令 |
