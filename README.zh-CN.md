...
> [!IMPORTANT]
> Lunil 当前版本是 **`0.6.0-alpha.1`**。编译器、托管运行时和完整 Lua 5.4 标准库
...
- **面向多层执行后端**：reference interpreter 与持久化 CIL AOT 已共享同一套 verified
  execution contract；分级 CoreCLR JIT 与 NativeAOT 构建集成继续复用该 ABI。
...
| JIT / AOT 后端 | 开发中 | deterministic persisted CIL AOT v1 已实现；分级 JIT 与构建期 NativeAOT 待完成 |
...
│   ├── Lunil.CodeGen.Cil/       # typed CIL plan、deterministic PE/PDB 与加载
