...
<p align="center">
  ...
  <a href="https://github.com/dlqw/Lunil/releases"><img alt="版本" src="https://img.shields.io/badge/version-0.7.0--alpha.3-7c3aed?style=flat-square"></a>
...
> [!IMPORTANT]
> 当前源码版本是 **`0.7.0-beta.1`**。`0.6.0` 开发线已在不可变的
...
> 通过，14 个公共 assembly/package 已由兼容性基线冻结。Beta 只接受加固工作。
...
| JIT / AOT 后端 | Beta 已验证 | Tier 1、精确数值 Tier 2 与带守卫的精确数值 loop OSR 已具备六 RID rollout 证据并默认自动启用；persisted CIL 已具备可验证、可回收的加载/执行路径与六 RID 生产性能门槛；managed semantic fallback 仍是实验性显式 opt-in |
| Compiler 产品 API | Beta 已冻结 | 统一的有界 lex/parse/bind/lower/verify 管线、不可变结果、阶段诊断、取消边界和 canonical source identity |
| Hosting 产品 API | Beta 已冻结 | 可复用 compile/execute 宿主、Trusted/Restricted/Deterministic 能力 profile 和运行时预算 |
| 注解产品 API | Beta 已冻结 | 共享有界 annotation lexer/type AST、LuaLS 默认 parser、legacy EmmyLua 兼容、未知标签保留、可配置诊断和 suppression |
| 类型与流分析 API | Beta 已冻结 | semantic type/type-pack、annotation declaration、constraint、CFG、函数/返回推断、nil/type/assert/判别字段收窄、definite assignment、unreachable、generic、source suppression 和确定性 widening 预算 |
| Workspace 产品 API | Beta 已冻结 | 稳定 module/source identity、可注入 resolver、静态/动态 require 分类、SCC fixed point、内容寻址缓存、最小失效、有界并行、取消与确定性合并 |
| CLI | Beta 已冻结 | 已打包的 `lunil` 工具，提供 `run`/`check`/`build`/`dump`、稳定退出码、text/JSON 诊断、stdin、响应文件、分层配置、workspace 解析、资源预算与 trusted/sandbox/deterministic profile |
| 稳定性契约 | Beta 预发布 | 功能/API 范围已冻结；进入 RC 前只继续兼容性、诊断、文档、可靠性和性能加固 |

### 当前后端就绪情况

| 执行路径 | Release 行为 | 进入 `0.7.0-beta.1` 时继承的就绪情况 |
| --- | --- | --- |
...
回归门槛。Beta 已冻结[0.7.0 路线图](docs/roadmap-0.7.0.md)中的编译器、分析、宿主、
CLI、conformance 与 package 产品面。
