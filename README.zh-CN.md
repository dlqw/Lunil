...
> [!IMPORTANT]
> Lunil 当前版本是 **`0.6.0-alpha.14`**。编译器、托管运行时和完整 Lua 5.4 标准库
...
| JIT / AOT 后端 | 预览 | 已通过资格验证的 Tier 1、精确数值 Tier 2 与带守卫的精确数值 loop OSR 均默认自动启用；persisted CIL 已具备可验证、可回收的加载路径与生产性能门槛；所有 managed fallback 路径继续保持显式 opt-in |
