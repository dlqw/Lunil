...
<p align="center">
  ...
  <a href="https://github.com/dlqw/Lunil/releases"><img alt="稳定版本" src="https://img.shields.io/badge/stable-0.18.1-16a34a?style=flat-square"></a>
...
| 静态分析 | Symbol、type、CFG、稳定 key、member/reference index、call graph、metatable/object model fact、class-factory 定义、closure upvalue 与 nil path 分析。 |
...
| 编辑器工具 | 自包含 LSP 3.17 server 与分平台 VS Code 插件，提供诊断、导航、rename、symbol、semantic token、hint、call hierarchy、workspace-symbol 搜索、class hierarchy 与 find-usages。 |
| 部署 | .NET 10 与 `netstandard2.1` 资产，NativeAOT、trimming、single-file、ReadyToRun、IL2CPP，以及六个桌面 RID 的 release bundle。 |

> [!TIP]
> 当前版本变更见 [0.18.1 Release](https://github.com/dlqw/Lunil/releases/tag/v0.18.1)，兼容性细节见
> [迁移指南](docs/migration-0.18.0.zh-CN.pub.md)。
...
```bash
gh release download v0.18.1 --repo dlqw/Lunil --pattern "*.nupkg" --dir .lunil-packages
```
...
```bash
dotnet tool install --global Lunil.Cli --version 0.18.1
lunil --version
```

```xml
<PackageReference Include="Lunil.Hosting" Version="0.18.1" />
```
...
| 示例 | 打开或运行方式 |
| --- | --- |
| [可移植 host](https://github.com/dlqw/Lunil/tree/v0.18.1/samples/Lunil.Portable.Hosting) | `dotnet run --project samples/Lunil.Portable.Hosting` |
| [静态分析嵌入](https://github.com/dlqw/Lunil/tree/v0.18.1/samples/Lunil.StaticAnalysis.Embedding) | `dotnet run --project samples/Lunil.StaticAnalysis.Embedding` |
| [Unity 2022.3](https://github.com/dlqw/Lunil/tree/v0.18.1/samples/Lunil.Unity.2022.3) | 直接用 Unity 2022.3 LTS 打开 |
| [Unity 6](https://github.com/dlqw/Lunil/tree/v0.18.1/samples/Lunil.Unity.6) | 直接用 Unity 6 打开 |
| [Godot 4.4](https://github.com/dlqw/Lunil/tree/v0.18.1/samples/Lunil.Godot.4.4) | 直接用 Godot 4.4.1 .NET 打开 |
| [Godot 4.6](https://github.com/dlqw/Lunil/tree/v0.18.1/samples/Lunil.Godot.4.6) | 直接用 Godot 4.6.3 .NET 打开 |
