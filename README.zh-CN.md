...
- **面向多层执行后端**：reference interpreter、持久化 CIL AOT 与 profile-guided
  CoreCLR Tier 1/Tier 2 JIT 已共享同一套 verified execution contract；NativeAOT
  构建集成继续复用该 ABI。
...
| JIT / AOT 后端 | 开发中 | persisted CIL AOT v1 与 profile-guided CoreCLR Tier 1/Tier 2 JIT 已实现；OSR 与构建期 NativeAOT 待完成 |
