...
<p align="center">
  ...
  <a href="https://github.com/dlqw/Lunil/releases"><img alt="版本" src="https://img.shields.io/badge/version-0.6.0--alpha.1-7c3aed?style=flat-square"></a>
...
> [!IMPORTANT]
> Lunil 当前版本是 **`0.6.0-alpha.2`**。编译器、托管运行时和完整 Lua 5.4 标准库
> 已经可用并具有较完整的测试，但公共 API、完整 Lua 官方测试套件覆盖和优化执行后端尚未全部完成。
...
| JIT / AOT 后端 | 开发中 | persisted CIL AOT v1、profile-guided CoreCLR Tier 1/Tier 2 JIT、显式开启的实验性 loop OSR 与六 RID NativeAOT 构建集成已实现；缓存产品化待完成 |
