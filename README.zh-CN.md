...
`0.10.0-alpha.2` 预发布版另外提供显式 Lua 5.1–5.5 契约。源码和版本化 PUC Lua 二进制 chunk
...
> `0.10.0-alpha.2` 预发布版暴露 Lua 5.1–5.5 的显式版本身份和独立 PUC chunk adapter，
> 同时保持 Lua 5.4 为默认版本。
...
- 语言目标：默认 Lua 5.4.8；`0.10.0-alpha.2` 预发布版提供显式 Lua 5.1–5.5 目标。
- 运行时目标：.NET 10。
