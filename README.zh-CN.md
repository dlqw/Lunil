...
> [!IMPORTANT]
> 当前源码版本是 **`0.8.0-rc.1`**；稳定版 `0.7.0` 及其 API/package baseline 保持
> 不可变。本候选版不带产品变化地继承 Beta 冻结的 interpreter、Tier 1/Tier 2 JIT、
> loop OSR、公共 API、assembly、package、IR/profile 和证据 schema 范围。Lua persisted/static
> AOT 继续保持移除，.NET NativeAOT 解释器兼容继续受支持；稳定版前只接受 release blocker。
...
| 执行后端 | `0.8` RC | 合格 interpreter、Tier 1/Tier 2 JIT 与 loop OSR；Lua persisted/static AOT 已在 `0.8.0-alpha.12` 移除 |
...
| 稳定性契约 | 活跃 RC | 稳定 `0.7.0` 保持冻结；`0.8.0-rc.1` 原样继承已验收 Beta 产品，只接受 release blocker |

### 当前后端就绪情况

| 执行路径 | Release 行为 | `0.8.0-rc.1` 就绪情况 |
| --- | --- | --- |
...
稳定 `0.7.0` 证据继续作为回归下限。`0.8.0-rc.1` 在不改变产品代码、公共 API 或 package
范围的前提下晋级已验收 Beta；详见其[更新日志](changelogs/0.8.0-rc.1.md)。Lua AOT 移除等
...
当前源码版本是 **`0.8.0-rc.1`**。稳定版 `0.7.0`、其 tag 与 `api/0.7.0` 保持
不可变；该稳定线的向后兼容修复使用 `0.7.1`。完整 `0.8` 功能、公共 API、assembly
和 package 范围继续冻结在 `api/0.8.0` 中。RC 只接受 release blocker；经验收的候选版
将不带产品变化地晋级稳定版。
