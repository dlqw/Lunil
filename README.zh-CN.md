...
<p align="center">
  ...
  <a href="https://github.com/dlqw/Lunil/releases"><img alt="版本" src="https://img.shields.io/badge/version-0.9.0--alpha.1-7c3aed?style=flat-square"></a>
...
> [!IMPORTANT]
> 当前源码版本是 **`0.9.0-alpha.1`**；稳定版 `0.8.0` 及其 API/package baseline 保持
> 不可变。本 Alpha 删除 Loop OSR 专属旧 IL emitter，使 Tier 2 函数入口与 OSR 回边入口共享
> 同一 numeric proof/emission pass。Loop OSR 的隔离 benchmark/configuration 入口和 canonical
> managed fallback 继续保留。Lua AOT 仍保持移除，.NET NativeAOT 解释器兼容继续受支持。
...
| 执行后端 | `0.9` Alpha | Interpreter、Tier 1 与 Tier 2；loop OSR 是共享 numeric pass 的 Tier 2 回边入口 |
...
| 稳定性契约 | 活跃 `0.9` Alpha | 稳定 `0.8.0` 保持冻结；`0.9.0-alpha.1` 接受已审核的后端架构工作 |

### 当前后端就绪情况

| 执行路径 | Release 行为 | `0.9.0-alpha.1` 就绪情况 |
| --- | --- | --- |
...
稳定 `0.8.0` 继续作为产品与回归下限。`0.9.0-alpha.1` 在不改变稳定 `api/0.8.0`
契约的前提下开始[后端架构工作](changelogs/0.9.0-alpha.1.md)。Lua AOT 移除等 breaking
变更详见 [`0.8.0` 迁移指南](docs/migration-0.8.0.md)。
...
当前源码版本是 **`0.9.0-alpha.1`**。稳定版 `0.8.0`、其 tag 与 `api/0.8.0` 保持
不可变；向后兼容修复使用 `0.8.1`。`0.9` 的公共 API 与架构范围在 Alpha 期间开放，
首个里程碑统一 Tier 2/Loop OSR numeric specialization。
