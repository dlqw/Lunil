...
[简体中文](signed-patch-bundles.zh-CN.pub.md)
...
steps, see [Deploy signed patch bundles](deploy-signed-patch-bundles.pub.md). For the transaction and
durability model, see [How hot-update publication works](signed-patch-publication.pub.md).
...
and compiles by descending hotness during preparation. Its `ExecutorOptions` use
`LuaHostJitWarmupOptions` for per-module function, duration, Tier 2, and profiled-function limits.
Warmup does not create closures, run loaders, mutate live state, or enter the update window. Total
and per-module function/duration budgets apply.
...
Compiled code uses the existing content-addressed JIT cache and remains bounded by
`LuaHostJitOptions.MaximumCodeCacheBytes` and normal eviction.
...
The `patch` command group contains `pack`, `verify`, `inspect`, `dry-run`, and `diff`. The trust-store
schema rejects unknown properties, duplicate key ids, malformed or non-P-256 keys, empty validity
windows, and more than 1,024 keys. Only `pack` reads private keys; verification and preflight use
public keys. The CLI does not download patches, manage a CDN, or store signing keys. Exact syntax and
options are in the [CLI reference](cli.pub.md); the production sequence is in
[Deploy signed patch bundles](deploy-signed-patch-bundles.pub.md).
