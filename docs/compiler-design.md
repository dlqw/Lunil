...
返回后不再有旧 generation delegate 执行。persisted/static AOT 没有绑定动态 generation，仍依靠其
exact module/artifact identity 与现有 guard。
...
当前 persisted CIL AOT 使用 artifact schema/codegen v2 与 Runtime ABI-v4，生成 deterministic PE、
deterministic Portable PDB、canonical module 只读资源和 JSON manifest。manifest 固化 artifact schema、
IR/Runtime ABI/codegen 版本、module content ID、options/profile fingerprint、profile policy、checksum、
function/shard map 与 numeric-region map。exact-module `LuaJitModuleProfile` 通过同一套
backend-neutral numeric IL stream 驱动 Reflection.Emit Tier 2 与 metadata PE emission；持久化方法使用
真实 unboxed `long`/`double`/`bool` local，并在 guard、预算和 safepoint 退出时保留精确 canonical PC。
canonical shard 在进入或离开数值区域的边界拆分，使 dispatcher 能优先选择 specialized method，
同时保留覆盖所有 PC 的兼容 fallback。Portable PDB 同时保存 Lua source checksum、source-line
sequence point，以及 IL offset 到 canonical/logical PC 的 Lunil custom debug record。普通 CoreCLR
通过显式 loader 先验证 manifest、版本、profile identity、numeric-region 不变量与资源 checksum，
再使用 collectible `AssemblyLoadContext` 注册 function delegate；PE 尾部的 deterministic SHA-256
footer 覆盖完整映像，损坏、错 ABI、错 module/profile identity 或不匹配的 PDB 只返回稳定诊断。
...
profile-guided persisted numeric region 的 identity、guard、safepoint 与兼容策略见
[ADR 0015](adr/0015-persisted-profile-guided-numeric-regions.md)。
