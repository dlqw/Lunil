...
control flow 与数值 helper。`NumericForPrepare`/`NumericForLoop` 和固定/开放 vararg 均在同一个方法内
继续 canonical PC dispatch；数值循环不再每轮返回 scheduler。exact emitter 对整个函数 CFG 做 fail-closed
coverage，任何未原生发射的 instruction 都会以 `JIT2106` 在入队前拒绝，而不是安装包含逐指令 slow path
的“部分 Tier 2”方法；tag guard 失败从原 canonical PC deopt。包含 table PIC、known call、constant
...
`Tier2Invocations` 与 method-entry 计数保持兼容。
