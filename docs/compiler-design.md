...
`SideEffectsMayHaveOccurred`。成功提交后旧 canonical module 的 JIT cache 被失效。该 idle
边界只是 state observation，不是跨 executor 的并发 barrier；suspended coroutine、旧 closure、
已有 local function reference、watcher 与跨版本 PC/profile remap 留给 versioned concurrent
invalidation 设计。完整契约见 [ADR 0016](adr/0016-idle-module-reload.md)。
