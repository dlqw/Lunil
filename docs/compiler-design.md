...
精确 debug hook 和 backend backedge probe 仍返回共享 scheduler。普通紧凑批次最多执行 32 条
指令后推进 GC 并派发 finalizer；every-allocation stress 或已存在 pending finalizer 时立即进入
safe point。指令数组通过受支持的 immutable collection marshal 取得只读 backing array 并以
`in LuaIrInstruction` 传递，不改变 canonical IR 的 immutable ownership。完整边界见
