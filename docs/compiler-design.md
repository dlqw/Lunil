...
与 exact-numeric Tier 2。Loop OSR 已具有独立的 exact-numeric 生产代码形态，但
`EnableLoopOsr=false` 仍保持显式 opt-in；所有 managed fallback 也必须由第二个开关明确开启。
...
Loop OSR 由 `EnableLoopOsr` 独立控制并默认关闭。CFG 分析只接受 target 为基本块 leader、header
...
显式开启 OSR 后会为每个函数执行一次 `LuaJitLoopOsrEligibility` 分析。只有完整循环可生成
`GuardedExactNumericCil` 且至少包含一个 exact numeric hotspot 时才自动编译；允许的组合包括
...
已完成操作。OSR 编译与 Tier 1/Tier 2 共用 bounded queue、并发去重、LRU code-byte budget、module
...
code kind、专门化与 guard 数量；`JIT3101`–`JIT3104`、accepted/rejected 事件和独立 statistics 公开资格。

无 natural loop 或所有 loop 均被拒绝的函数在首次正常 entry 分析后进入永久 fast-rejection 状态，
后续 instruction/backedge 不再运行 analyzer、dictionary lookup 或锁。M6 的 managed prototype 曾只有
...
specialized emission pipeline；只有动态代码可用且 `EnableLoopOsr=true` 时才预热 OSR specialized
emitter，避免未开启 OSR 或 NativeAOT 进程承担初始化成本。
