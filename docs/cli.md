...
- `--lua-version 5.1|5.2|5.3|5.4|5.5` selects the explicit language contract. Lua 5.4 is the
  default. Each selected version uses its own source/compiler/runtime and binary-chunk adapter;
  unsupported version identities fail with a diagnostic rather than silently using Lua 5.4.
