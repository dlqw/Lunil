...
[简体中文](migration-0.10.0.zh-CN.pub.md)
...
## 1. Configure Lua 5.1 function environments
...
## 2. Make `require` use the state version
...
## 3. Replace removed Lua AOT APIs

The persisted/static Lua AOT product, disk cache, generated static registry, and `Lunil.Build` package are unavailable. Execute runtime-compiled source with `LuaHost`, `LuaInterpreter`, or `LuaJitExecutor`; distribute precompiled input as verified portable PUC chunks. See the [0.8.0 migration guide](migration-0.8.0.pub.md) for removed API names and CLI inputs.
