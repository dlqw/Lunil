...
- 基线解释器、基于 CoreCLR CIL 的分级 JIT、带守卫的 Tier 2 loop OSR 和 .NET NativeAOT 兼容；
...
7. JIT/profile/workspace 缓存损坏、缺失或版本不匹配只能导致重新分析或编译，不能改变程序语义。
...
diagnostic、阶段间 cancellation 检查和不可变 compilation result。所有 CLI、Hosting 与 workspace
源码输入必须复用该入口，不能维护第二套 parse/bind/lower 拼装逻辑。后续注解、类型、流与模块分析均接入
...
中引入隐藏 mutable global state。Hosting 复用同一 workspace API；`0.7` 的 Build 产品曾复用该
边界，但已随 `0.8.0-alpha.12` 的 Lua AOT 删除而移除。runtime `package`/`require` 行为及
canonical IR 保持不变。
...
源码 lowering 和 chunk 导入必须产生相同的 `LuaIrModule`/`LuaIrFunction`。函数使用连续编号，保存父函数编号、参数数、vararg 标记、最大寄存器窗口、二进制常量、upvalue 来源、线性指令和从指令重建的基本块。当前格式版本为 3；版本 2 增加 `SetTop`，版本 3 增加 binary source name、定义行、逐指令 source line/logical PC、二进制 upvalue 名与 canonical debug-local 范围。持久化缓存或 runtime specialization profile 必须把该版本纳入兼容键。
...
`0.8.0-alpha.8` 为每次成功的 `require` 建立 cache-validated `LuaModuleRecord`。记录保存
module name、preload/Lua file/custom searcher provenance、loader、loader data、当前 cache 值、
可选 canonical IR 和单调 revision；内部 registration 通过 `LuaHandle` 持有三个 Lua 值，公开
snapshot 只提供借用视图。查询记录时必须和当前 `package.loaded[name]` 比较；Lua 代码直接
evict/replace cache 后，陈旧记录在下一次查询时被移除并释放 root。`load`、`loadfile` 与
`dofile` 不创建记录，因为它们没有 `require` 的具名 cache ownership。

`LuaHost.ReloadModule` 只在 state idle 时执行，并与同一 host 的 execute/dispose 共享串行 gate。
Lua file loader 从记录或 override path 重新读取和编译；preload 与 custom searcher 重放已保留的
loader 和 loader data，不重新运行可能已变化的 searcher chain。流程分为 prepare、candidate
execute 和 commit：候选完成前旧 cache 始终可见，成功后再选择 replace、保留 table identity 的
raw entry/metatable patch，或 host callback policy。兼容的主 loader upvalue 按 index、name、
source kind/index 与 IR kind 复用旧 cell；不兼容 layout 不猜测映射。

编译失败不执行 candidate。执行或 policy 失败恢复旧 cache、require record、revision、table
patch 与复用 upvalue 的值；但 loader 或 callback 已经造成的 global、reachable table、I/O 与
host-service 副作用不具备通用事务回滚能力，因此结果显式报告
`SideEffectsMayHaveOccurred`。成功提交后旧 canonical module 的 JIT cache 被失效。alpha.8 的 idle
边界只是 state observation，不是跨 executor 的并发 barrier；当时 suspended coroutine、旧 closure、
已有 local function reference 与跨版本 profile remap 尚未迁移。完整 v1 契约见
[ADR 0016](adr/0016-idle-module-reload.md)。

### 8.8 function version 与并发失效

`0.8.0-alpha.9` 将 Lua closure 的代码身份拆为 atomic `LuaFunctionSlot` 与 immutable
`LuaFunctionVersion`。version 固化 canonical module/function、generation、lexical key、完整 upvalue
layout fingerprint 及其专属 constant/table-allocation/frameless-call cache。每次 Lua frame 入栈先
snapshot version，之后 PC、指令、常量、debug source/line 与 GC root traversal 都从 snapshot 读取；
因此 active 或 suspended coroutine 在 reload 后仍从旧 PC 继续旧代码，而新调用读取 slot 当前值。

file reload 对旧 cache 与 candidate cache 的 raw table key/value、metatable 及 closure-upvalue 图做
cycle-safe traversal，但只收集分别属于旧/新 canonical module 的 closure，避免共享 `_ENV` 把其他
module 或旧 alias 误当作 candidate。logical key 由 lexical parent 与 sibling ordinal 构成；只有 key
相同且 upvalue name/source kind/source index/IR kind 完全一致时，旧 slot 才以 compare-exchange 发布
candidate function 的 successor version。原 closure identity 与 upvalue cell 不变；missing、layout
mismatch 与 concurrent publication 都进入结构化 migration result。lexical sibling 插入会保守地移动
后续 ordinal，因此这种编辑可能拒绝本可人工映射的函数，但不会猜测错误身份。

动态 JIT 为每个 module content ID 维护 generation coordinator。compiled entry 必须先获取对应
generation lease；entry ABI 与 Loop OSR backedge/safepoint 检测 stale token，并统一以
`BackendInvalidated` 返回共享 scheduler。`Invalidate` 先禁止旧 generation 新入口并清除 Tier 1、
Tier 2 与 OSR method，再等待旧 lease 全部退出；`ClearCache` 与 dispose 采用相同 quiescence 原则。
返回后不再有旧 generation delegate 执行。

`LuaJitProfileRemapper` 提供跨 module content ID 的保守 profile 转换。source payload 必须先通过原
module 的 schema、checksum、identity 与 profile validation；函数按 lexical key 对齐，且 parameter/
vararg、upvalue fingerprint、instruction count 和每条 `Opcode/A/B/C/D` 必须完全相同。常量值变化
不影响 site 坐标，因此可复用；任何 PC 或 operand 变化只清空对应目标函数。self-module Lua call
target 按 lexical identity 重写到目标 content ID/function ID，外部 target 与 table shape 继续由运行时
guard 保护。完整决策见 [ADR 0017](adr/0017-versioned-invalidation-and-function-slots.md)。watcher
仍由宿主在外部适配，不进入 runtime。
...
当 `RuntimeFeature.IsDynamicCodeSupported` 或 `IsDynamicCodeCompiled` 为 false 时 JIT 自动禁用。
机器码不持久化；owner-free profile 只在调用方显式导出时跨进程保存。
...
allocation、代码形态与专门化/deopt 数量。register liveness 由 module owner-scoped weak cache 在 Tier 1、Tier 2 与 Loop OSR 之间共享。
...
预算退出与 safepoint 边界统一提交。纯数值 backedge 使用最大 1024 次的 local countdown，包含可能分配的
table side operation 时收紧为 256；非 poll 回边只累计
...
64 位 ABI 上的隐藏返回缓冲区；所有剩余生成后端绑定 `long` factory overload。详见
...
`0.8.0-alpha.15` 将相同的“提示只选择 guard，不充当事实”原则扩展到 table 与非数值 operand。
Tier 2 对 exact integer array key、interned string field、boolean/nil/string equality 和 string
relational operation 生成 guarded CIL；未 profile 的 primitive unary/binary 仍在当前 compiled entry
执行 exact-kind guard 和通用 slow path，而不是因为缺少样本退出整个方法。table PIC 最多四路，
string entry 同时验证 weak table/key identity、shape/storage/metatable version 和 opaque entry handle；
metamethod-absence entry 验证 weak metatable identity 与 content version。所有 miss 均可回到已有 raw
probe，真正的 table/key/metatable/result-kind 失配才从原 canonical PC deopt。

compiler-proven table identity 使用 CFG forward must-analysis：`NewTable` 创建 token，`Move` 传播，
任意 register write 清除，join 只保留所有 predecessor 一致的 token。只有带该 token 的 table site 才能
把 table 引用提升到 compiled-entry local；定义指令再次执行会清空 local。verified numeric region 还可
把这类 integer 或 interned-string access 作为 tagged side operation，同时保留 induction、arithmetic、
comparison 与 accumulator 的 CLR locals。side operation 不自行 reserve 或提交 PC；hot block/cold tail 的
既有计费是唯一预算来源，失败在副作用前回滚并物化。含 table side operation 的 region 最迟每 256 个
backedge poll，string constant 每个 compiled entry 只物化一次。详见
[ADR 0021](adr/0021-bounded-table-operand-pics.md)。

生产 `TablePicHits`/`TablePicMisses` 是低成本采样估计：首个事件精确记录，稳定事件按 256 个聚合；
`TablePicInvalidations` 始终精确。测试 counter sink 保持逐事件计数。whole-function Tier 2 还要求至少
一个完整 Tier 1 invocation 已结束，避免首个 invocation 的 backedge hotness 用部分 profile 发布整个
函数；首轮长循环的当前帧接管仍由 Loop OSR 负责。
...
[ADR 0009](adr/0009-loop-osr-auto-default-rollout.md)。当前跨平台性能数据统一维护在
[性能文档](performance.md)，不在架构文档中复制实现细节。
...
Reflection.Emit sink 消费经过验证的 plan；尚未 lowering 的 opcode 生成显式 deopt exit，不能落入
...
`0.8.0-alpha.12` 删除 persisted/static Lua AOT、metadata/PE emitter、artifact/manifest、loader、
executor、disk cache、`Lunil.Build` 与对应 benchmark/release gate。它们不再是当前架构层。
历史设计和性能证据仍保留在 [ADR 0010](adr/0010-persisted-cil-aot-performance-productionization.md)
和 [ADR 0015](adr/0015-persisted-profile-guided-numeric-regions.md)，但均已 superseded。

.NET NativeAOT 是宿主发布方式而非 Lua 后端：compiler、workspace、runtime、CLI 与 interpreter
仍需通过 trimming/NativeAOT。动态代码不可用时 `LuaJitExecutor` 确定性回退解释器，不初始化
Reflection.Emit、不采样 profile，也不尝试动态加载 CIL。旧 CLI/config/environment AOT target
返回稳定 `LUNIL0006`，不得静默改走 chunk 或 JIT。
...
`0.8.0-alpha.8` 在该 capability/profile 边界上增加独立的执行后端选择。`LuaHost` 的默认
...
当前执行后端没有持久化 artifact disk cache。Tier 1/Tier 2/Loop OSR 的 plan、liveness、delegate
与 code-byte budget 均绑定 module owner，并使用 weak ownership；module 回收、executor dispose、
`Invalidate` 或 `ClearCache` 后不得被缓存反向保活。机器码只存在于当前进程。

显式 `LuaJitModuleProfile` 导入/导出只包含 owner-free、版本化 observation。导入必须校验 module
content identity、function identity、policy/schema version 和完整 canonical instruction stream；失配时
fail closed 到空 profile，不得改变 Lua 语义。Compiler/workspace 自身的内容缓存继续由对应产品
边界管理，不与 JIT delegate 生命周期混合。
...
- interpreter、Tier 1、Tier 2、loop OSR 与 NativeAOT interpreter fallback 结果一致；
...
runtime 的遗留 heap 状态带入下一样本。六 RID aggregate 对缺失、重复、schema 不兼容或结果错误
保持 fail closed。当前发布数据、图表和复现命令见[性能文档](performance.md)。
...
6. 实现 Tier 1/Tier 2 CIL JIT、PIC、deopt 与 OSR。
7. 接入 NativeAOT/trimming compatibility、sandbox、宿主 API 和发布工具链。
8. 完成全平台 conformance、fuzz、性能回归及稳定发布。
