...
Resource rule 覆盖 `Coroutine`、`Timer`、`EventSubscription`、`Task` 和 `HostResource`，
disposition 包括 `Continue`、`Cancel`、`Restart`、`Drain` 与 `RejectIfActive`。对于 runtime-owned coroutine，
...
不可复制的引擎对象、native session、file/socket wrapper 或其他必须跨 generation 保留单一 userdata
identity 与 owner 的 host 生命周期，应使用 `HostResource`。Lua 需要通过 exact allowlist 访问 CLR member
时，用 `LuaClrBridge.CreateStableResource` 创建初始 userdata；否则 host 也可以直接把
`LuaPatchStableResourceHandle` 放入自己创建的 userdata：
...
旧 module cache 的 `StatePath` 必须指向该 live handle，且 `ResourceId` 必须完全一致。Candidate 只需保留
相同 parent path，可以使用非 resource placeholder。`Continue` 会把旧 userdata 安装到 candidate graph，
因此外部 registry 与 Lua alias 继续观察同一 identity 和 ownership；后续 module、barrier、health 或
distributed rollback 会恢复旧 graph，而不会 dispose resource。Candidate loader 不得重复构造 native
resource。

Host 使用底层对象期间应持有 `LuaPatchStableResourceHandle.AcquireLease()`。Handle dispose 后会立即拒绝
新 caller，但 owned `IDisposable` 或 `IAsyncDisposable` 要等全部 lease 关闭后才真正释放；通过 bridge
发起的 CLR member invocation 与 event subscription 会自动持有 lease。存在任何显式 lease 时，
`RejectIfActive` 会失败。`Cancel`、`Restart` 与 `Drain` 的外部副作用取决于应用，仍必须使用可逆 resource
adapter。
