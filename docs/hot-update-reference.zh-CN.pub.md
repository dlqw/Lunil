...
[English](hot-update-reference.pub.md)

本参考页定义 Lunil 的签名 patch manifest、信任与 replay 契约、preparation 与 commit resource、migration
rule、rollout state、持久 store 和 telemetry。操作步骤见 [如何部署签名 Patch Bundle](hot-update.zh-CN.pub.md)。
...
hotness 降序编译。它不创建 closure、不运行 loader、不修改 live state，也不进入 update window。整个
patch 与 per-module function/duration budget 同时适用。
...
`NotApplicable`。已编译 code 使用现有有界 content-addressed JIT cache。
...
CLI 不下载 patch、不管理 CDN，也不存储 signing key。
