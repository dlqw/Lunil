...
public sealed class LuaCodegenAbiV2Tests
{
    ...
    {
        var (_, _, frame) = CreateFrame();

        Assert.True(LuaCodegenAbiV2.CanSkipClose(frame, 1));
        frame.ToBeClosedSlots.Add(frame.Base + 2);
        Assert.False(LuaCodegenAbiV2.CanSkipClose(frame, 1));
        Assert.True(LuaCodegenAbiV2.CanSkipClose(frame, 3));
    }
